            ...
            optional_data: ClientCoreOptionalData {
                ...
                early_capability_flags: {
                    let mut early_capability_flags = ClientEarlyCapabilityFlags::VALID_CONNECTION_TYPE
                        ...
                        // PATCH (slimRDM): upstream defines this flag but never
                        // sets it, so a server never opens the graphics pipeline
                        // channel. Hosts running the WDDM RDP display driver have
                        // no legacy output path at all and paint nothing without
                        // it — a live session on a blank canvas. See
                        // docs/superpowers/plans/2026-09-17-rdp-graphics-pipeline.md
