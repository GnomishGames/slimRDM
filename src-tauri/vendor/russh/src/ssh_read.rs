    ...
    pub async fn read_ssh_id(&mut self) -> Result<&[u8], Error> {
        ...
        loop {
            ...
            if ssh_id.bytes_read > 0 {
                ...
                // SSH-1.99 (RFC 4253 §4.2) signals SSH-1/2 dual support; treat it
                // as SSH-2 since we send SSH-2.0 and the server will proceed with SSH-2.
                if i >= 8 && (ssh_id.buf.get(0..8) == Some(b"SSH-2.0-")
                    || ssh_id.buf.get(0..8) == Some(b"SSH-1.99"))
                {
                    ssh_id.sshid_len = i;
                    #[allow(clippy::indexing_slicing)] // length checked
