...
pub fn decode_rlex(data: &[u8]) -> DecodeResult<RlexData> {
    ...
    // Compute bit widths. A single-entry palette needs zero bits for the stop
    // index, which leaves all eight for the suite depth — it does NOT mean the
    // packed byte is absent.
    ...
    // "run lengths only" consumed the packed byte as a run, which yielded one
    // extra segment and therefore one extra suite pixel per segment — decoding
