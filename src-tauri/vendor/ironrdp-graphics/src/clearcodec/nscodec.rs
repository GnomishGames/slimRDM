...
pub(crate) fn decode(
    ...
    // Chroma planes are half-resolution on each axis when subsampled.
    let (chroma_width, chroma_height) = if chroma_subsampling == 0 {
        (width, height)
    } else {
    ...
    for row in 0..height {
        ...
        for col in 0..width {
            ...
            let y = i32::from(luma[row * width + col]);
            ...
            output[dst + 3] = alpha
                .as_ref()
                .and_then(|a| a.get(row * width + col).copied())
                .unwrap_or(0xFF);
        }
    }
