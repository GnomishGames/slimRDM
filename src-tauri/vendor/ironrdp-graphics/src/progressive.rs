...
pub struct ProgressiveDecoder {
    ...
    /// Band layout from the most recent CONTEXT block, whichever context id it
    /// arrived under.
    ///
    /// PATCH (slimRDM): the flag describes the codec negotiation, not one
    /// context. Windows establishes it once and then opens further context ids
    /// without repeating the CONTEXT block, so keying the fallback strictly by
    /// context id rejected every frame under a new id with
    /// `MissingBlock("CONTEXT")` — leaving those tiles unpainted. This is the
    /// same server leniency the per-id fallback below already accommodates,
    /// widened to match; FreeRDP keeps the flag per decoder instance.
    last_use_reduce_extrapolate: Option<bool>,
}
...
impl ProgressiveDecoder {
    ...
    pub fn new() -> Self {
        Self {
            ...
            last_use_reduce_extrapolate: None,
        }
    }
    ...
    pub fn decode_bitmap(
        ...
        let use_reduce_extrapolate = match blocks.iter().find_map(|block| match block {
            ...
            Some(v) => {
                self.last_use_reduce_extrapolate = Some(v);
                v
            }
            None => self
                .contexts
                ...
                .or(self.last_use_reduce_extrapolate)
